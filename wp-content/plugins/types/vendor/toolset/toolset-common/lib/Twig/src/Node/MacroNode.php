 ...
 */
class MacroNode extends \OTGS\Toolset\Twig\Node\Node
{
    const VARARGS_NAME = 'varargs';
    public function __construct($name, \OTGS\Toolset\Twig_NodeInterface $body, \OTGS\Toolset\Twig_NodeInterface $arguments, $lineno, $tag = null)
    {
        foreach ($arguments as $argumentName => $argument) {
            if (self::VARARGS_NAME === $argumentName) {
                throw new \OTGS\Toolset\Twig\Error\SyntaxError(\sprintf('The argument "%s" in macro "%s" cannot be defined because the variable "%s" is reserved for arbitrary arguments.', self::VARARGS_NAME, $name, self::VARARGS_NAME), $argument->getTemplateLine(), $argument->getSourceContext());
            }
        }
...
\class_alias('OTGS\\Toolset\\Twig\\Node\\MacroNode', 'OTGS\\Toolset\\Twig_Node_Macro');
