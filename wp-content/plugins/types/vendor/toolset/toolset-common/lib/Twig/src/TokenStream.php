...
class TokenStream
{
    ...
    public function __construct(array $tokens, $name = null, $source = null)
    {
        if (!$name instanceof \OTGS\Toolset\Twig\Source) {
            ...
            $this->source = new \OTGS\Toolset\Twig\Source($source, $name);
        } else {
            $this->source = $name;
        }
        $this->tokens = $tokens;
    ...
    public function next()
    {
        if (!isset($this->tokens[++$this->current])) {
            throw new \OTGS\Toolset\Twig\Error\SyntaxError('Unexpected end of template.', $this->tokens[$this->current - 1]->getLine(), $this->source);
        }
    ...
    public function expect($type, $value = null, $message = null)
    {
        ...
        if (!$token->test($type, $value)) {
            $line = $token->getLine();
            throw new \OTGS\Toolset\Twig\Error\SyntaxError(\sprintf('%sUnexpected token "%s"%s ("%s" expected%s).', $message ? $message . '. ' : '', \OTGS\Toolset\Twig\Token::typeToEnglish($token->getType()), $token->getValue() ? \sprintf(' of value "%s"', $token->getValue()) : '', \OTGS\Toolset\Twig\Token::typeToEnglish($type), $value ? \sprintf(' with value "%s"', $value) : ''), $line, $this->source);
        }
        $this->next();
        return $token;
    }
    ...
    public function look($number = 1)
    {
        if (!isset($this->tokens[$this->current + $number])) {
            throw new \OTGS\Toolset\Twig\Error\SyntaxError('Unexpected end of template.', $this->tokens[$this->current + $number - 1]->getLine(), $this->source);
        }
    ...
    {
        return \OTGS\Toolset\Twig\Token::EOF_TYPE === $this->tokens[$this->current]->getType();
    }
    /**
     * @return Token
     */
...
\class_alias('OTGS\\Toolset\\Twig\\TokenStream', 'OTGS\\Toolset\\Twig_TokenStream');
